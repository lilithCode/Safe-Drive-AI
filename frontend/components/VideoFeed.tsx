...
interface Props {
  ...
  driveTime: number;
  safetyScore: number;
  nerdMode: boolean; // New prop to control HUD vs Nerd view
}
...
export default function VideoFeed({
  ...
  // State for dynamic latency simulation
  const [latency, setLatency] = useState(32);

  useEffect(() => {
    aiDataRef.current = aiData;
    // Simulate real-time processing fluctuation when data arrives
    if (aiData) {
      setLatency(Math.floor(Math.random() * (36 - 28 + 1) + 28));
    }
  }, [aiData]);
  ...
  return (
    <div className="bg-[#3D2B1F] rounded-2xl border border-[#4D392C] p-2 sm:p-4 shadow-2xl transition-all duration-500 h-full flex flex-col">
      <div className="relative flex-1 w-full rounded-xl overflow-hidden bg-black shadow-inner ring-1 ring-[#4D392C]">
        ...
        {/* --- NERD MODE ONLY: FLOATING TELEMETRY --- */}
        {nerdMode && (
          <div className="absolute bottom-4 left-4 z-30 flex gap-2 animate-in fade-in slide-in-from-left duration-500">
      ...
      {/* --- NERD MODE ONLY: BOTTOM BOXES --- */}
      {nerdMode && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-4 animate-in fade-in slide-in-from-bottom duration-500">
          ...
          <StatCard icon={<Zap size={14} />} title="AI Latency" value={`${latency}ms`} accent="#A3B18A" />
        </div>
      )}
    </div>
  );
...
function StatCard({ icon, title, value, accent }: any) {
  return (
    <div className="bg-[#2A1E16]/40 p-4 rounded-xl border border-[#4D392C] flex flex-col gap-1 transition-all group">
