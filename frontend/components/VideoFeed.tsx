...
//   driveTime: number;
//   safetyScore: number;
//   nerdMode: boolean; // Controls Driver HUD vs Technical Telemetry
// }
...
//       if (data) {
//         if (data.drowsy || data.phone_detected) {
//           activeColor = warmRed;
//           labelText = "CRITICAL_INTERVENTION";
//         } else if (data.head_distracted || data.yawning) {
...
//       scanGrad.addColorStop(0.5, `${activeColor}22`);
//       scanGrad.addColorStop(1, `rgba(0,0,0,0)`);
//       ctx.fillStyle = scanGrad;
//       ctx.fillRect(0, scanY - 60, canvas.width, 120);

//       // 2. High Vis Target Vector Box (Always Visible for Driver Reference)
//       ctx.strokeStyle = activeColor;
//       ctx.lineWidth = 1.5;
//       ctx.strokeRect(canvas.width * 0.25, canvas.height * 0.2, canvas.width * 0.5, canvas.height * 0.65);
      
//       // Target Reticle Corners
//       const gap = 25;
//       ctx.beginPath();
//       // Top Left
//       ctx.moveTo(canvas.width * 0.25 - 5, canvas.height * 0.2 + gap); ctx.lineTo(canvas.width * 0.25 - 5, canvas.height * 0.2 - 5); ctx.lineTo(canvas.width * 0.25 + gap, canvas.height * 0.2 - 5);
//       // Top Right
//       ctx.moveTo(canvas.width * 0.75 + 5, canvas.height * 0.2 + gap); ctx.lineTo(canvas.width * 0.75 + 5, canvas.height * 0.2 - 5); ctx.lineTo(canvas.width * 0.75 - gap, canvas.height * 0.2 - 5);
//       ctx.stroke();

//       // 3. Render Landmark Meshes Only in Nerd Mode
//       if (nerdMode && data && data.face_detected && data.landmarks && data.landmarks.length > 0) {
...
//           const nodeSize = 1.6 + fastPulse * 1.0;
//           ctx.beginPath();
...
//         {/* HUD Bottom Left: Telemetry - Conditionally rendered for developers */}
//         {nerdMode && (
//           <div className="absolute bottom-4 left-4 z-30 flex gap-2 animate-in fade-in duration-300">
...
//       {/* Bottom Stats Grid - Swapped to render purely under developer calibration */}
//       {nerdMode && (
//         <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
