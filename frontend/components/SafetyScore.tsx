...
export default function SafetyScore({ score, nerdMode }: SafetyScoreProps) {
  ...
  const getStatusColor = () => {
    if (score >= 80) return "#E8B06F"; // Amber
    if (score >= 50) return "#8C6A53"; // Muted Brown/Taupe
    return "#D9534F";                  // Alert Red
  };

  const color = getStatusColor();

  const statusText =
    score >= 80 ? "SYSTEM SECURE" : score >= 50 ? "CAUTION — DISTRACTION" : "CRITICAL — PULL OVER";

  return (
    <div className="bg-[#3D2B1F] p-6 rounded-[2.5rem] border border-[#4D392C] flex flex-col items-center shadow-xl relative overflow-hidden transition-all duration-500 min-h-[300px] justify-center">
      
      {/* Reduced background glow intensity and blur for a subtle "aura" */}
      <div
        className="absolute top-0 w-full h-32 blur-[60px] opacity-[0.03] pointer-events-none"
        style={{ backgroundColor: color }}
      ></div>

      <h3 className="text-[10px] font-black text-[#E8B06F] uppercase tracking-[0.3em] w-full text-left mb-6 z-10 opacity-80">
        Safety Rank
      </h3>

      {/* The Gauge */}
      <div className="relative w-56 h-32 flex justify-center overflow-hidden z-10">
        <svg className="w-full h-full" viewBox="0 0 100 60">
          ...
          {/* Active Progress Path - Removed heavy drop-shadow */}
          <path
            d="M 10,50 A 40,40 0 0,1 90,50"
            fill="none"
            stroke={color}
        ...
        <div className="absolute bottom-0 flex flex-col items-center">
          ...
          <span className="text-[8px] font-black text-[#BFA899] uppercase tracking-[0.3em]">
            Score
          </span>
        </div>
      </div>

      <p className="text-[9px] font-black text-white/60 tracking-widest uppercase mt-6 z-10 text-center">
        {statusText}
      </p>

      {/* Mini Visual Segments */}
      <div className="mt-4 flex justify-center gap-1.5 z-10">
        {[...Array(5)].map((_, i) => (
          <div 
            key={i} 
            className="w-6 h-1 rounded-full transition-all duration-500" 
            style={{ 
                backgroundColor: (roundedScore / 20) > i ? color : '#2B1D14',
                opacity: (roundedScore / 20) > i ? 0.8 : 0.4
            }} 
          />
        ))}
      </div>
      ...
      {/* Nerd Mode Detail Bar */}
      {nerdMode && (
        <div className="w-full mt-6 flex items-center gap-3 z-10 animate-in fade-in">
          <div className="flex-1 h-1 bg-[#2B1D14] rounded-full overflow-hidden">
            <div
              className="h-full rounded-full transition-all duration-500"
              style={{ width: `${roundedScore}%`, backgroundColor: color }}
            />
